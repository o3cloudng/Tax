...
def agency_dashboard(request):
    ...
    if infrastructures.filter(one_month).exists():
        infrastructures_month = companies.filter(one_month)
        infrastructures_month = infrastructures_month.count() / infrastructures_month.count() * 100
    else:
        infrastructures_month = []
        
    # Demand Notices
    demand_notices = DemandNotice.objects.all()
    demand_notice = demand_notices.count() # demand_notices_month

    if demand_notices.filter(one_month).exists():
        demand_notices_month = companies.filter(one_month)
        demand_notices_month = demand_notices_month.count() / demand_notices_month.count() * 100
    else:
